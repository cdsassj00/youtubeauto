...
export async function generateScript(params: {
  ...
  const user = [
    ...
    '- 중간 씬들은 bullets / diagram / comparison / quote / code 를 다양하게 섞어 지루하지 않게 한다. 한 영상에 diagram 이나 bullets 만 반복되지 않게, 5가지 타입을 고루 번갈아 쓴다.',
    ...
    '- illustration 필드는 title/outro 씬에서만 실제로 화면에 렌더링된다(다른 타입은 코드로 그린 화면이 대신 쓰임). 그러니 title/outro 의 illustration 에 특히 공을 들여라: 그 영상의 핵심 주제를 한눈에 보여주는, 구체적이고 실제 존재하는 사물/화면/장면 하나를 정한다.',
    '- visual="title" 인 씬은 영상 첫 씬뿐 아니라 중간에도 나올 수 있는데(장 전환용), 어느 위치든 illustration 필드를 절대 비워두면 안 된다 — 비워두면 heading 문구만으로 그림을 만들게 되어 엉뚱하게 뭉뚱그려진다. visual="title"·"outro" 인 모든 씬은 illustration 을 반드시 구체적으로 채워라.',
    '- illustration 은 "직접적·직관적으로 그대로" 보여주는 흑백 등각(isometric) 삽화의 영어 시각 묘사여야 한다. 실제로 설명하는 사물·행동·화면을 3/4 등각 각도의 입체 형태로 구체적으로 그린다(예: "서버"면 데이터센터 랙, "데이터베이스"면 원통형 DB 아이콘과 표, "로그인"이면 자물쇠와 아이디/비밀번호 입력창).',
    '- illustration 에서 금지: 뇌+회로, 빛나는 전구, 추상적으로 떠다니는 점/네트워크, 로봇손과 사람손의 악수, 빛나는 구體/오브, 홀로그램 뇌, 마법같은 빛 효과 — 이런 뻔한 "AI 클리셰" 이미지는 절대 쓰지 마라. 항상 실제 사물(노트북 화면, 종이 서류, 사무실 책상, 특정 제품 UI, 실제 도구)로 구체화해라.',
    '- 한 영상 안에 illustration 이 실제로 그려지는 씬은 title/outro 뿐이지만(보통 2~4개), 그 안에서도 "사무실 책상 + 노트북"만 반복하지 마라 — 매번 같은 그림처럼 보이는 가장 큰 원인이다. 용어/개념을 다루는 추상적인 주제(신조어, 정의, 비교, 트렌드 같은 소재)라면 책상+노트북 대신 다음처럼 서로 다른 구체적 장면에서 매번 골라 써라: 칠판/화이트보드 클로즈업, 인쇄된 사전·백과사전 펼친 페이지, 게시판에 핀으로 꽂힌 인덱스카드 여러 장, 컨베이어 벨트 위 상자에 라벨을 찍는 도장/스탬프 장치, 여러 방향을 가리키는 표지판 무리, 책장에서 한 권만 비스듬히 뽑아낸 모습, 회의실 화이트보드에 적힌 벤 다이어그램, 계산기와 영수증처럼 수치를 보여주는 장면, 돋보기로 인쇄물의 한 문단을 확대해 보는 장면. 같은 영상 안의 title 과 outro 는 반드시 서로 다른 장면을 골라라(같은 소재를 두 번 쓰지 말 것).',
    '- title(제목)은 클릭하고 싶되 정확한 40자 이내.',
    '- thumbnailHeadline: 썸네일에 크게 박을 아주 짧고 강한 후킹 문구. 한 줄에 5~9자씩 최대 2줄 분량(전체 18자 이내). 제목을 그대로 줄인 게 아니라 다음 후킹 공식 중 하나를 실제로 써서 새로 뽑아라: (1)의외의 숫자/사실 하나를 던지기(예: "본문 등장 딱 1번" 같은 실제 수치를 구체적 대상과 함께), (2)정곡을 찌르는 도발적 단정(예: "하네스 엔지니어링? 그냥 설정"), (3)정면 질문(예: "이 말 모르면 뒤처질까요?"), (4)극명한 대비(예: "설정 vs 엔지니어링"). "~완벽정리", "~진짜 원리" 같은 뻔한 클리셰 문구는 피하고, 그 영상 안의 구체적 사실/숫자/단정을 직접 박아라.',
    '- thumbnailHeadline 은 그림만 보고도 무슨 소리인지 바로 알 수 있어야 한다 — "그거", "이거" 같은 지시대명사만으로 시작해 대상이 안 보이면 절대 안 된다(예: "그거 설정이다"는 무엇이 "그거"인지 썸네일만 봐선 알 수 없어 틀렸다). 반드시 핵심 키워드(예: "하네스 엔지니어링", 다루는 용어/개념 자체)를 문구 안에 직접 넣어 그 자체로 완결되게 써라.',
