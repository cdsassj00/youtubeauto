...
export async function generateScript(params: {
  ...
  const user = [
    ...
    '- 반드시 다음 넷 중 하나의 "긴장"을 담아라: (1) 대비 — 상식과 어긋나는 둘을 붙인다("같은 값, 2배 실력"). (2) 의외성 — 예상을 깬 사실을 선언한다("이번엔 값을 안 올렸다"). (3) 궁금증 — 답이 궁금해지는 질문("왜 값을 안 올렸을까"). (4) 이득 선언 — 시청자가 얻는 것을 단정한다("공짜로 2배 빨라졌다").',
    '- thumbnailBadge: 문구에서 뺀 대상을 대신 담는 짧은 배지(제품·회사·기술 이름, 2~12자. 예: "Claude Opus 5", "RAG"). 썸네일 구석에 작게 들어간다. 문구 안에 이미 대상이 자연스럽게 들어갔다면 비워도 된다.',
