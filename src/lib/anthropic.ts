...
export async function generateScript(params: {
  ...
  const user = [
    ...
    ...(isFootage ? [
      ...
      '- ★이 영상은 배경이 전부 실사 영상·사진이다★ 그 위에 큰 글씨, 또는 코드로 그린 등각 도식이 얹힌다. 코드·불릿 화면은 존재하지 않는다. visual 은 title / image / quote / diagram / comparison / outro 여섯 가지만 쓴다.',
      '- ★diagram·comparison 은 실사 배경 위에 도식이 떠오르는 화면이다★ 전체 씬의 20~30% 정도만 써라. 매 씬마다 도식이 뜨면 눈 둘 곳이 없어져 오히려 산만해진다. 나머지는 image·quote 로 두어 실사와 큰 글씨가 숨 쉴 자리를 만든다.',
      '- diagram 은 "여러 요소가 순서·관계로 이어지는" 내용에만 쓴다(단계 흐름, 구조, 순환). nodes 2~6개와 edges 로 표현하고 node.id 는 짧은 영숫자, label 은 한국어 8자 이내로 짧게.',
