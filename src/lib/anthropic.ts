...
export async function generateScript(params: {
  ...
  const user = [
    ...
    '- illustration 필드는 title/outro 씬(영상 전체에서 딱 2개, 첫 씬과 마지막 씬)에서만 실제로 화면에 렌더링된다(다른 타입은 코드로 그린 화면이 대신 쓰임). illustration 필드를 절대 비워두면 안 된다 — 비워두면 heading 문구만으로 그림을 만들게 되어 엉뚱하게 뭉뚱그려진다.',
