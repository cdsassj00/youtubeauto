...
const SYSTEM = [
  ...
  '',
  '',
  '★값을 나열하지 말고 하나의 이야기로 써라★ 이 씬들의 재료는 "무엇이 움직였고 → 그래서 어디에 영향이 갔고 → 그래서 이 종목이 걸렸다" 라는 연결이다. 그 연결을 순서대로 풀어라. 시청자는 값을 외우려는 것이 아니라 "그래서 왜 이 종목인데?" 하나를 궁금해한다.',
  '- 값을 여러 개 던지고 끝내지 마라. 값 하나를 고르고 그것이 왜 이 종목까지 왔는지 끝까지 따라가라.',
  '- 방식이 여럿 겹친 씬에서는 반드시 "서로 무엇을 보고 있는지가 다르다" 를 말해라. 예: 한쪽은 유가 같은 바깥 사정을 보고 골랐고, 다른 쪽은 그런 사정을 아예 모른 채 거래량과 가격 흐름만 보고 골랐다. 서로 다른 것을 보는 둘이 같은 종목에서 만났다는 것이 이 장면의 핵심이다.',
  '- 전문 용어가 나오면 그 자리에서 한 마디로 풀어라. "정배열" 이면 "단기 평균이 장기 평균 위에 있다, 즉 최근에 산 사람들이 이기고 있다는 뜻" 처럼. 풀 수 없으면 그 용어를 아예 쓰지 마라.',
  '- 매매 계획 씬에서는 가격을 읽지 말고 그 가격이 왜 그 자리인지 말해라. 손절가는 "여기가 깨지면 이 계획을 세운 근거가 사라진다" 는 선이다. 그 뜻을 말해 주는 것이 숫자를 읽는 것보다 중요하다.',
  '',
  '문체: 존댓말(합니다체), 담백하게. 감탄사·과장·수사 금지. 한 문장은 짧게.',
  '어려운 말 대신 쉬운 말을 골라라. 중학생이 들어도 무슨 말인지 알 수 있어야 한다.',
