...
- figure  : { type:"figure", art:"이름", kicker, claim:"화면 제목(<em>강조</em> 가능)", lead:"보조설명" }
            // 엔진에 미리 그려둔 설명 그림을 불러 쓴다. 원고에 [도식: art=이름] 표시가 있으면
            // 그 자리에 반드시 이 슬라이드를 넣고 art 를 그 이름으로 맞춰라.
            // 쓸 수 있는 art: ram(켜놓은 창들이 램 위에 올라가 있음) / poweroff(전원 끄기 전후 비교)
            //                ddr(DDR4 대비 DDR5 두 배 막대) / ai(AI가 아는 것 전부를 칩이 꺼내 봄)
            //                hbm(보통 램의 좁은 길 vs 쌓아 올린 HBM의 넓은 길 + 칩 옆 배치)
            //                cost(한 층이 잘못되면 덩어리 전체를 버림)
            // 목록에 없는 art 이름을 지어내지 마라. 그림이 안 나온다.
...
export async function generateDeck(params: {
  ...
  // 사용자가 짧은 "주제" 대신 완성된 원고/브리핑을 통째로 붙여넣는 경우가 있다.
  // 그때 이걸 그냥 주제로 넘기면 모델이 자기 마음대로 다시 써서 원문이 사라진다.
  // 일정 길이를 넘으면 "따라야 할 원고"로 취급해 구성·사실·순서를 보존하게 한다.
  const isBrief = topic.trim().length > 800;
  ...
  const system = [
    ...
    isBrief
      ? [
          '',
          '★이번 입력은 "주제"가 아니라 사용자가 직접 쓴 완성 원고다★',
          '- 원고의 구성(도입 → 각 절 → 클로징)과 절의 순서를 그대로 따라라. 절을 빼거나 순서를 바꾸지 마라.',
          '- 원고에 있는 사실·고유명사·수치는 그대로 쓴다. 원고에 없는 수치·날짜·제품명을 새로 지어내지 마라.',
          '- 원고의 주장과 결론을 바꾸지 마라. 네가 더 좋다고 생각하는 논지로 갈아끼우는 것은 금지다.',
          '- 네가 할 일은 "다시 쓰기"가 아니라 "화면으로 옮기기"다: 각 문단을 어떤 슬라이드 타입으로 보여줄지 고르고,',
          '  나열은 metrics/cmp 로, 단계·구조는 nodes/flow 로, 핵심 선언은 claim/big 으로 배치한다.',
          '- 말투는 원고의 존댓말 톤을 유지한다. 화자 인사말(오프닝·클로징의 이름 소개)도 살린다.',
          `- 분량이 ${targetChars}자에 못 미치면 원고의 내용을 "풀어서" 채워라(원고에 이미 있는 항목을 예시·부연으로 확장).`,
          '  원고에 없는 새 사실을 넣어서 늘리는 것은 금지.',
        ].join('\n')
  ...
  const user = isBrief
    ? [
        '=== 사용자가 직접 쓴 원고 (이 내용을 화면으로 옮겨라) ===',
        topic,
        '=== 원고 끝 ===',
        // 원고가 있을 때 웹서치 결과는 보조 자료일 뿐 — 원고와 어긋나면 원고를 따른다.
        research?.trim() ? `\n[참고용 최신 정보 — 원고와 충돌하면 원고를 우선한다]\n${research.trim()}` : '',
        '위 원고의 구성과 사실을 그대로 지키면서, 위 형식의 JSON 을 출력하라.',
      ].join('\n')
    : [
        `주제: ${topic}`,
