...
export async function generateCourseMeta(
  ...
  const user = [
    ...
    '- thumbnailHeadline: 8~14자. ★썸네일에서 가장 큰 글씨로 들어가는 문구다★ 이 회차에서 가장 궁금하게 만드는 한마디를 써라. 명사 나열 말고 질문이나 단언으로. (예: "챗봇과 뭐가 다른가", "프롬프트는 숨어 있다") ' +
      '시리즈 공통 문구("돈 주고도 못 듣는 강의")는 썸네일 구석에 따로 박히니 여기 넣지 마라. 40편이 한 시리즈인데 이 문구까지 서로 비슷하면 목록에서 전부 같은 영상으로 보인다 — 이 회차에서만 나오는 구체적인 것을 잡아라.',
