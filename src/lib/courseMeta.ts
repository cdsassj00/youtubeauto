...
export async function generateCourseMeta(
  ...
  const system = [
    ...
    '낚시성 표현("충격", "이것만 알면", "99%가 모르는")을 쓰지 마라. 실무자가 보는 강의다.',
  ...
  const user = [
    ...
    '',
    '요구사항:',
    `- title: 45~60자. "${input.moduleLabel}" 같은 모듈 표시는 넣지 마라(코드가 따로 붙인다). 이 강의의 실제 내용을 담되, 검색해서 찾을 만한 말로 써라.`,
    ...
    '- thumbnailHeadline: 12자 안쪽. 이 강의의 핵심을 한마디로. 제목을 그대로 반복하지 마라.',
