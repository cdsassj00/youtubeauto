...
export const CourseMetaSchema = z.object({
  ...
  /** 노란 배지로 박을 짧은 값(숫자·결과). 없으면 빈 문자열. */
  thumbnailBadge2: z.string().default(''),
});
...
export async function generateCourseMeta(
  ...
  const user = [
    ...
    '- thumbnailHeadline: 두 줄. 줄바꿈(\\n)으로 나누고 한 줄은 9자 이내. ★썸네일에서 가장 큰 글씨이고, 이 영상의 조회수는 사실상 이 한 줄이 정한다★',
    '  · ★반드시 둘 중 하나는 들어가야 한다★ (1) 누구나 아는 이름 — 엑셀, CSV, ChatGPT, 클로드, 크롬, 파이썬, API 키, 주피터, 노션 같이 ' +
      '보는 순간 무엇인지 아는 것. (2) 숫자 — "10만 줄", "3줄", "2.3GB", "7분". 둘 다 없으면 다시 써라.',
    '  · ★모르는 개념어를 주인공으로 쓰지 마라★ 하네스, 서브에이전트, 온톨로지, 임베딩, MCP 같은 말은 모르는 사람에게는 궁금하지도 않다. ' +
      '"하네스 없을 땐?" 은 하네스를 아는 사람만 읽는 말이라 아무도 안 눌렀다. 그런 회차는 그 개념이 해결하는 상황을 아는 말로 바꿔 써라 — ' +
      '"하네스" 대신 "인터넷 막힌 회사에서", "서브에이전트" 대신 "AI 여러 개를 한 번에".',
    '  · ★개념 질문은 최악이다★ "플러그인이 뭔데", "왜 코딩 에이전트인가", "챗과 워크는 뭐가 다른가" 는 목차이지 썸네일이 아니다. ' +
      '얻는 것·잃는 것·바뀌는 것을 말해라. 좋은 예: "엑셀 10만 줄 / 3줄만 붙여라", "API 키 / 여기 넣으면 털림", "스킬 20개 / 한 번에 설치".',
    '  · ★강조할 낱말을 ** 로 감싸라★ 그 부분만 다른 색으로 칠해진다. 한 줄에 한 덩어리만. 예: "**엑셀 10만 줄**\\n3줄만 붙여라".',
    '  · ★강의에 없는 말은 안 된다★ 세게 쓰되 지어내지 마라. 숫자도 자막에 실제로 나온 것만 쓴다.',
    '  · 시리즈 공통 문구("돈 주고도 못 듣는 강의")는 썸네일 구석에 따로 박히니 여기 넣지 마라.',
    '- thumbnailBadge2: 8자 이내. 썸네일에 노란 배지로 박을 짧은 값 — 숫자나 결과가 있으면 그것(예: "3줄이면 끝", "2.3GB"). 마땅한 것이 없으면 빈 문자열.',
