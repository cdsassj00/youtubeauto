/* ssjhtmlvideo 렌더러 — deck.json(각 비트에 say/spoken) → 나레이션 싱크 mp4.
   각 비트 ElevenLabs TTS → 오디오 길이로 dwell 결정 → deck-timed 프레임렌더(디스크에 안 쌓고
   ffmpeg 로 바로 파이프) → 나레이션 mux. 긴 영상도 디스크 부담 없이 매끄럽게.
   사용: ELEVENLABS_API_KEY=.. ELEVENLABS_VOICE_ID=.. node render.mjs deck.json out.mp4
   환경변수: W,H(기본 1920x1080) FPS(24) NARRATION_SPEED(1) */
...
if (!deckPath || !outPath) { console.error('사용: node render.mjs deck.json out.mp4'); process.exit(1); }
...
// 3) HTML 조립 (엔진별)
const b64 = fs.readFileSync(path.join(HERE, 'assets', 'pretendard.woff2')).toString('base64');
...
let html;
if (ENGINE === 'signal') {
  const engine = fs.readFileSync(path.join(HERE, 'engines', 'deck-signal.js'), 'utf8');
  const monoPath = path.join(HERE, 'assets', 'mono.woff2');
...
<body><script>${inj}</script><script>${engine}</script></body></html>`;
  ...
  const lib = fs.readFileSync(path.join(HERE, 'assets', 'three.min.js'), 'utf8');
  const engine = fs.readFileSync(path.join(HERE, 'engines', 'deck-timed.js'), 'utf8');
